...
class DatasetGen(object):
    ...
    def __init__(self, args):
        ...
        self.taskcla = [int(self.num_classes/self.num_task) for t in range(self.num_task)]
        ...
        task_ids = np.split(np.random.permutation(self.num_classes),self.num_task)
    ...
    def get_meta_loader(self, meta_dataset):
        create_bookkeeping(meta_dataset, self.ways, self.meta_label)

        meta_transforms = [
                    l2l.data.transforms.NWays(meta_dataset, self.ways),
                    l2l.data.transforms.KShots(meta_dataset, 2*self.shots),
